...
class IPV_Admin {
	...
	private function __construct() {
		...
		$plugin = IPV::get_instance();
		$this->plugin_slug = $plugin->get_plugin_slug();

		add_action( 'admin_init', array( $this, 'ipv_init' ) );
	...
	public function ipv_init() {
		
		// Check for wp_footer
		$this->check_footer();
		register_setting( 'ipv-settings-group', 'ipv_script' );
		register_setting( 'ipv-settings-group', 'ipv_on_pages' );
		register_setting( 'ipv-settings-group', 'ipv_off_pages' );
	}
	...
	public function enqueue_admin_styles() {
		...
		if ( $this->plugin_screen_hook_suffix == $screen->id ) {
			wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), IPV::VERSION );
		}
	...
	public function enqueue_admin_scripts() {
		...
		$screen = get_current_screen();
		if ( $this->plugin_screen_hook_suffix == $screen->id ) {
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/admin.js', __FILE__ ), array( 'jquery' ), IPV::VERSION );
		}
